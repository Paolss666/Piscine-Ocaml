...
object
  ...
  method to_string = Printf.sprintf "Molecule: %s (Formula: %s)" "Oxygen"
    (compute_formula [new Atom.oxygen; new Atom.oxygen])
  method equals (other : molecule) = other#formula = "O2"
end

class carbon_monoxide =
object
  inherit molecule "Carbon Monoxide" [new Atom.carbon; new Atom.oxygen]
  method to_string = Printf.sprintf "Molecule: %s (Formula: %s)" "Carbon Monoxide"
    (compute_formula [new Atom.carbon; new Atom.oxygen])
  method equals (other : molecule) = other#formula = "CO"
end

class carbon_solid =
object
  inherit molecule "Carbon" [new Atom.carbon]
  method to_string = "Molecule: Carbon (Formula: C)"
  method equals (other : molecule) = other#formula = "C"
end
